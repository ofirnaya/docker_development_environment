...
INTELLIJ_LINK="https://download-cdn.jetbrains.com/idea/ideaIC-2021.3.tar.gz" \
...
yum install -y openssh-server openssh-client xorg-x11* wget sudo python3 python3-devel java-11-openjdk java-11-openjdk-devel nc git && \
...
pip3 install pyspark==${SPARK_VERSION} jupyter jupyterlab && \
pip3 install --upgrade jedi==0.17.2 && \
pip3 install kafka-python jupyterlab && \
